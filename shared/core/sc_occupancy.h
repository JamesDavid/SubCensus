...
typedef struct {
    ...
    float peak_rssi; /* dBm */
    float occupancy; /* 0..1 fraction of samples above threshold */
...
typedef struct {
    int32_t freq_hz;
    float peak_rssi;
    float noise_floor; /* min RSSI seen (floor proxy) */
    int32_t above;
    int32_t total;
    int32_t crossings;
    int64_t last_seen;
...
typedef struct {
    ...
    float threshold_dbm; /* noise_floor + margin (adaptive per band) */
    float occupancy;
} ScWatchlistEntry;
